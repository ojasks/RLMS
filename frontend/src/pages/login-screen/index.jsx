  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userData = {
        ...
        formAccess: response.data.formAccess,
        token: response.data.token,
        macAddress: response.data.macAddress || '00:1B:44:11:3A:B7',
        ipAddress: response.data.ipAddress || '192.168.1.100',
        network: response.data.network || 'SecureLab-Network',
        loginTime: new Date().toISOString()
      };
      ...
      if (userData.role === 'normal') {
        navigate('/dashboard-home');
      } else if (userData.role === 'group_head' || userData.role === 'admin') {
        navigate('/response-analytics');
      }
