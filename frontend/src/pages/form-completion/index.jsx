...
const FormCompletion = () => {
  ...
  const totalQuestions = useMemo(() => 
    formSections.reduce((total, section) => total + section.questions.length, 0),
    [formSections]
  );
  ...
  const estimatedTimeRemaining = useMemo(() => 
    Math.max(0, Math.round((formData.estimatedTime * (1 - progressPercentage / 100))),
    [formData.estimatedTime, progressPercentage]
  ));
  ...
  const validateCurrentSection = useCallback(() => {
    ...
    return Object.keys(errors).length === 0;
  }, [currentSection, formSections, responses, validationErrors]);
